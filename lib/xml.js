...
XML.parse = function parse(xmlText, options = {}) {
    ...
    if (matches = /<\?xml(.*?)\?>/.exec(pocket.xmlText)) {
        ...
        Object.assign(data, parseAttributes(attributes, XML._ATTR_SIGN));
    }

    while (matches = new RegExp(template, 'img').exec(pocket.xmlText.slice(posLast))) {
        ...
        if (singleTag) {
            parseSingleTag.call(data, tagName, tagAttributes, posLast, posStart);
            showTree && logTree(pocket.depth, tagName, singleTag);
        }
        else if (closedTag) {
            parseClosedTag.call(data, tagName, posStart, posLast);
            showTree && logTree(pocket.depth, tagName, closedTag);
        }
        else {
            showTree && logTree(pocket.depth, tagName);
            parseOpenedTag.call(data, tagName, tagAttributes, posLast, posStart);
        }
    }

    delete data[XML._POCKET_NAME];

    // return Object.setPrototypeOf(data, XML.prototype);
    return data;
}
...
function parseSingleTag(tagName, tagAttributes, posLast, posStart) {
    const attributes = parseAttributes(tagAttributes);
    const pocket = this[XML._POCKET_NAME];
    ...
    if (pocket.depth > 0) {
        ...
        parentNode.outsideText += pocket.xmlText.substring(parentNode.lastPosition, posStart);
        parentNode.lastPosition = posLast;
        parentNode.hasTags = true;
    }
}

function parseOpenedTag(tagName, tagAttributes, posLast, posStart) {
    ...
    if (pocket.depth > 0) {
        ...
        const parentNode = pocket.datamap[pocket.depth - 1];
        parentNode.outsideText += pocket.xmlText.substring(parentNode.lastPosition, posStart);
        parentNode.lastPosition = posStart;
        parentNode.hasTags = true;
    }
    ...
    pocket.datamap[pocket.depth] = {
        ...
        data: attributes,
        tagName,
        posStartTag: posStart,
        posStartText: posLast,
        lastPosition: posLast,
        outsideText: ''
    }
...
function parseClosedTag(tagName, posStart, posLast) {
    const pocket = this[XML._POCKET_NAME];
    ...
    if (pocket.depth > 0) {
        ...
        if (currentNode.posStartTag === parentNode.lastPosition) {
            currentNode.outsideText += pocket.xmlText.substring(currentNode.lastPosition, posStart);
            ...
            if (/\S/.test(currentNode.outsideText) && currentNode.hasTags) {
                currentNode.data._innerText = pocket.xmlText.substring(currentNode.posStartText, posStart).trim();
                currentNode.data._outerText = pocket.xmlText.substring(currentNode.posStartTag, posLast).trim();
            }
        }

        if (!currentNode.hasTags) {
            const currData = parentNode.data[currentNode.tagName];
            const innerText = pocket.xmlText.substring(currentNode.lastPosition, posStart);
