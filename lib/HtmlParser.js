...
// Content beginning
HtmlReader.BGN_CONTNENT = 0;

// Content ending
HtmlReader.END_CONTNENT = 1;

// Tag beginning
HtmlReader.BGN_TAG = 2;

// Tag ending
HtmlReader.END_TAG = 3;

HtmlReader.TAG_OPENED = String.fromCharCode(0x3C);
HtmlReader.TAG_CLOSED = '</';
HtmlReader.TAG_ENDED = String.fromCharCode(0x3E);


/**
 * Wrapper for a searching result of the HtmlReader
 */
...
HtmlReader.ParseResult = ParseResult;
...
function search(selector, searchParams, firstMatch = false) {
  ...
  if (searchParams instanceof ParseResult) {
    tagStart = searchParams.coords[HtmlReader.BGN_CONTNENT] || searchParams.coords[HtmlReader.BGN_TAG];
    tagEnd = searchParams.coords[HtmlReader.END_CONTNENT] || searchParams.coords[HtmlReader.END_TAG];
  }
  ...
  while (matches = new RegExp(template, 'img').exec(self.html.substring(tagStart, tagEnd))) {
    ...
    if (HtmlReader.TAG_OPENED == tagDefine) {
      isPaired && tagDepth++;
    ...
    else if (HtmlReader.TAG_CLOSED == tagDefine) {
      tagDepth--;
      ...
      data.coords[HtmlReader.END_CONTNENT] = tagStart - tagHtml.length;
      data.coords[HtmlReader.END_TAG] = tagStart;
